...
`data/version.json` is deterministic content metadata: schemaVersion, counts,
file manifest, indexes, sourceDigest, bundle hashes and verification versions.
`npm run generate` checks it along with bundles; `--write` regenerates all
outputs from one validated candidate. `npm run version-info` uses that same
pipeline. There is no commit SHA in the tracked file (avoids self-reference).

Publication adds commit/date and `provenance` via `buildVersionInfo()`.
Explorer writes this to its public `version.json`; npm prepack writes
`package-snapshot.json` (exported as `drawtabdata/snapshot`). CI refuses
dirty data or generator inputs; local publication labels them explicitly.
See [CONSUMERS.md](CONSUMERS.md) for exact coverage and verification.
